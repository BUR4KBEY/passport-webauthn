...
Strategy.prototype.authenticate = function (req, options) {
  ...
  function validated(err, ok, ctx) {
    ...
    const allowedOrigins = (
      process.env.PASSPORT_FIDO2_WEBAUTHN_ALLOWED_ORIGINS ?? ''
    )
      .split(' ')
      .filter(Boolean);
    ...
    const allowedHashes = allowedOrigins.map(origin =>
      crypto.createHash('sha256').update(new URL(origin).hostname).digest()
    );
    ...
    if (clientData.type === 'webauthn.get') {
      ...
      if (!ctx.user) {
        ...
        if (userHandle && Buffer.compare(ctx.user.id, userHandle) != 0) {
          ...
          // same user.
          return self.fail(
            { message: 'User handle does not map to user' },
            403
          );
        }
      }
      ...
      if (
        !allowedHashes.some(buffer => buffer.equals(authenticatorData.rpIdHash))
      ) {
        ...
        if (authenticatorData.signCount && info && info.signCount) {
          self._verifySignCount(
            ...
            function (err, ok) {
              ...
              if (!ok) {
                return self.fail(
                  { message: 'Cloned authenticator detected' },
                  403
                );
              }
